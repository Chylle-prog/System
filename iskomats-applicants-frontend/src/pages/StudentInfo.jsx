...
// Concurrency limiter configured for optimal throughput
...
const compressImageForOcr = async (blobOrDataUrl, maxDim = 1024, quality = 0.80) => {
...
function toTwoDecimalsTruncated(val) {
  if (val === null || val === undefined) return null;
...
function extractGpaFromText(text, expectedGpa = null) {
  ...
  // 1. Explicit keyword match (supports "GPA: 3.5555", "GPA: 3.5481", "GPA: 3.4375", "GPA: 3.55", "GPA: 355", "GPA 35481")
    ...
    if (/(?:GPA|GWA|CWA|QPI|WEIGHTED\s*AVERAGE|GENERAL\s*WEIGHTED|FINAL\s*AVERAGE)/i.test(rawLine)) {
      ...
      if (kwMatch && kwMatch[1]) {
        ...
        if (rawVal.includes('.')) {
          ...
          if (!isNaN(val) && val >= 1.0 && val <= 5.0) return toTwoDecimalsTruncated(val);
  ...
  // 2. High-precision 4-decimal place term GPA on body text (e.g. 3.5555, 3.4375, 3.5481)
  ...
  if (rawSubjectTokens && rawSubjectTokens.length >= 2) {
    ...
    if (totalUnits > 0) {
      ...
      if (calcGpa >= 1.0 && calcGpa <= 5.0) {
        return toTwoDecimalsTruncated(calcGpa);
      }
    }
  }
  ...
  if (decimals.length > 0) {
    ...
    return toTwoDecimalsTruncated(mean);
  }
...
function gpaMatchesText(text, expectedGpa) {
  ...
  // 1. Exact numeric equality (e.g. 3.55 === 3.55)
  if (Math.abs(detVal - parsedInputGpa) < 0.00001) {
    return true;
  }

  // 2. Truncated 2-decimal comparison (e.g. user entered 3.55 and document extracted as 3.55)
  const inputTwoDecStr = toTwoDecimalsTruncated(parsedInputGpa);
  if (inputTwoDecStr && inputTwoDecStr === detectedGpaStr) {
    return true;
  }

  // 3. Fallback: Also match if user entered rounded or full decimal (e.g. 3.5555 or 3.56)
  const inputRounded2Dec = (Math.round(parsedInputGpa * 100) / 100).toFixed(2);
  const detRounded2Dec = (Math.round(detVal * 100) / 100).toFixed(2);
  if (inputRounded2Dec === detectedGpaStr || inputTwoDecStr === detRounded2Dec) {
    return true;
  }
...
const StudentInfo = () => {
    ...
    try {
      ...
      const evaluateVideoText = (textLogs) => {
        ...
        if (isIndigencyVideo) {
          ...
          const hasIndigencyInVideo = /certificate\s*of\s*indigency|katibayan\s*ng\s*kawalang|office\s*of.*indigency|\bindigent\b|\bindigency\b|indigent\s*families/i.test(rawCombined);
          const hasResidencyHeaderInVideo = /certificate\s*of\s*residency|certificate\s*of\s*residence|katibayan\s*ng\s*pagkapamayanan|office\s*of.*residency|barangay\s*residency/i.test(rawCombined);

          if (_vidIsResidency) {
            // Scholarship requires Certificate of Residency: reject if video shows Indigency
            if (hasIndigencyInVideo) {
              return {
                valid: false,
                isMatched: false,
            ...
            const hasResidencyKeywords = hasResidencyHeaderInVideo || /punong\s*barangay|office\s*of\s*the\s*punong\s*barangay|barangay|resident|residing|residence|residency/i.test(rawCombined);
            ...
            // Scholarship requires Certificate of Indigency: reject if video shows Residency header without Indigency
            if (hasResidencyHeaderInVideo && !hasIndigencyInVideo) {
              return {
                valid: false,
                isMatched: false,
            ...
            if (!hasIndigencyInVideo) {
              return {
                valid: false,
                isMatched: false,
                reason: "Indigency video proof failed: Video does not contain Certificate of Indigency keywords (Indigent/Indigency).",
        ...
        if (isSchoolIdVideo) {
          const isIndigencyDocVideo = /punong\s*barangay|barangay\s*inosluban|to\s*whom\s*it\s*may\s*concern|resident\s*of\s*this\s*barangay|residing\s*at|specimen\s*signature|kawalang\s*kabuhayan|certificate\s*of\s*indigency|certificate\s*of\s*residency|\bindigent\b|\bindigency\b/i.test(rawCombined) ||
      ...
      // --- RUN GOOGLE CLOUD VISION API OCR ON SEEKED FRAMES IN PARALLEL ---
          ...
          const results = await Promise.all(
            extractedFrames.map(async ({ time, canvas }) => {
              ...
              if (frameText && frameText.length >= 3) {
                return `[Frame at ${time.toFixed(1)}s]: "${frameText}"`;
              }
              return null;
            })
          );
          results.forEach(txt => {
            if (txt) accumulatedText.push(txt);
          });
        }
      } catch (ocrErr) {
  ...
  const cancelOcrScan = (docType) => {
    console.log('[OCR] Cancelling scan for:', docType);
    if (docType) {
      const token = activeOcrControllersRef.current[docType];
      if (token) {
        token.aborted = true;
        if (token.progressInterval) clearInterval(token.progressInterval);
        if (token.controller) {
      ...
      Object.keys(activeOcrControllersRef.current).forEach(k => {
        const token = activeOcrControllersRef.current[k];
        if (token) {
          token.aborted = true;
          if (token.progressInterval) clearInterval(token.progressInterval);
          if (token.controller) {
  ...
  async function performOcrVerification(docType, docParam, extraParams = {}, videoUrl = null, silent = false) {
    ...
    try {
      if (!silent) {
        ...
        scanToken.progressInterval = progressInterval;
      ...
      if (docType === 'SchoolID') {
        ...
        if (docType === 'Enrollment') {
          ...
          const hasExplicitIndigencyHeader = /certificate\s*of\s*indigency|katibayan\s*ng\s*kawalang|office\s*of.*indigency|\bindigent\b|\bindigency\b|indigent\s*families/i.test(imgDocText);
          const hasExplicitResidencyHeader = /certificate\s*of\s*residency|certificate\s*of\s*residence|katibayan\s*ng\s*pagkapamayanan|office\s*of.*residency|barangay\s*residency/i.test(imgDocText);

          let imageHasKeyword = false;
          let docTypeErrorMessage = null;

          if (isResidencyDoc) {
            // Scholarship requires Certificate of Residency: reject if image shows Indigency
            if (hasExplicitIndigencyHeader) {
              ...
              const residencyKeywords = ['residency', 'resident', 'residing', 'residence', 'pagkapamayanan', 'naninirahan', 'maninirahan', 'pamayanan'];
              imageHasKeyword = residencyKeywords.some(k => imgDocText.includes(k));
            }
          } else {
            // Scholarship requires Certificate of Indigency: reject if image shows Residency header without Indigency
          ...
          const hasIndigencyInVideo = /certificate\s*of\s*indigency|katibayan\s*ng\s*kawalang|office\s*of.*indigency|\bindigent\b|\bindigency\b|indigent\s*families/i.test(vidText);
          ...
          if (isResidencyDoc) {
            ...
            } else if (!hasIndigencyInVideo) {
              videoHasKeyword = false;
              videoTypeErrorMessage = 'Video proof mismatch: uploaded video does not show Certificate of Indigency keywords (Indigent/Indigency).';
            } else {
              const indigencyKeywords = ['indigency', 'indigent', 'kawalang', 'kapos', 'pagkakawalang'];
              videoHasKeyword = videoCheck
                          ...
                          <>
                            ...
                            {ocrVerified === 'verifying' && (
                              <div style={{ marginTop: '1rem' }}>
                                ...
                                <div style={{ marginTop: '0.75rem', padding: '0.75rem 1rem', background: '#eff6ff', borderRadius: '12px', border: '1px solid #bfdbfe', display: 'flex', alignItems: 'center', gap: '10px', color: '#1d4ed8', fontSize: '0.82rem', fontWeight: '700', width: '100%' }}>
