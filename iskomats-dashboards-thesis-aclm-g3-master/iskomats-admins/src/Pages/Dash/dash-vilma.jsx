...
export default function DashVilma() {
  ...
  const loadScholarships = async () => {
    try {
      console.log('Loading scholarships for program: vilma');
      const response = await scholarshipAPI.getByProgram('vilma');
  ...
  useEffect(() => {
    const loadApplicants = async () => {
      try {
        const response = await scholarshipAPI.getApplicants('vilma');
        ...
        console.error('Failed to load Vilma applicants:', error);
      }
    };
    ...
    if (token) {
      ...
      const unsubStatusUpdate = socketService.subscribe('applicant_status_update', (update) => {
        if (update.program !== 'vilma') return; // Only handle Vilma updates
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={(e) => { e.preventDefault(); manageTab === 'scholarship' ? saveScholarshipPost() : saveAnnouncement(); }}>
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder={manageTab === 'scholarship' ? "e.g. Vilma Scholarship 2026" : "e.g. System Maintenance"}
              required
            />
          </div>
