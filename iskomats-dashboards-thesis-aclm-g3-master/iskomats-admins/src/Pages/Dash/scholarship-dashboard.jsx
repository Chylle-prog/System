...
export default function ScholarshipDashboard({
  ...
  const [inboxTab, setInboxTab] = useState('applicants'); // applicants | superadmin
  const [superadminMessages, setSuperadminMessages] = useState([]);
  const [superadminReplyText, setSuperadminReplyText] = useState('');
  ...
  const renderInbox = () => {
    ...
    <div className="flex flex-col h-[calc(100vh-6.5rem)] sm:h-[calc(100vh-8rem)] bg-gradient-to-br from-gray-50 to-blue-50/30 animate-in fade-in duration-300">
      ...
      {/* Inbox Tab Switcher (only for provider admins with a super admin room) */}
      {isSuperAdminRoomActive && (
        <div className="flex gap-2 mb-3 flex-shrink-0">
          <button
            type="button"
            onClick={() => setInboxTab('applicants')}
          ...
          <button
            type="button"
            onClick={() => setInboxTab('superadmin')}
          ...
          >
            <FaInbox className="text-xs" /> Super Admin Chat
          </button>
        </div>
      )}

      {inboxTab === 'superadmin' && isSuperAdminRoomActive ? (
        // ===== SUPER ADMIN PRIVATE CHAT PANEL =====
        <div className="flex-1 bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-gray-100 bg-gray-50 flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-sm text-gray-900">Super Admin</h3>
              <p className="text-[10px] text-gray-400">Private channel — only you and the super admin can see these messages</p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 bg-white">
            {superadminMessages.length > 0 ? (
                    ...
                    <div className={`max-w-[85%] rounded-2xl p-3 sm:p-4 shadow-sm border ${
                      ...
                      <div className="flex items-center justify-between mb-1.5 gap-4">
                        <span className={`font-semibold text-xs ${isMe ? 'text-white/90' : 'text-[#800020]'}`}>
                          {isMe ? 'Me' : (msg.username || 'Super Admin')}
                        </span>
                        <span className={`text-[10px] flex items-center gap-1 ${isMe ? 'text-white/70' : 'text-gray-400'}`}>
                          <FaClock className="text-[9px]" /> {formatSADate(msg.timestamp)}
                        </span>
                      </div>
              ...
              <div className="flex flex-col items-center justify-center h-full text-gray-400 py-8">
                <FaInbox className="text-3xl mb-2 text-gray-200" />
                <p className="text-xs">No messages yet. The super admin can reach you here.</p>
              </div>
            )}
          </div>
          ...
          <div className="p-3 sm:p-4 border-t border-gray-100 bg-gray-50">
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={sendSuperAdminReply}
                disabled={!superadminReplyText.trim()}
                className="px-4 py-2 sm:px-5 sm:py-3 rounded-xl bg-[#800020] text-white font-semibold hover:bg-[#650018] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 text-xs sm:text-sm flex-shrink-0"
              >
                <FaPaperPlane />
              </button>
            </div>
