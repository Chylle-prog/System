...
def batch_convert_videos(current_user_id, pro_no, role):
    ...
    """
    try:
        print("[BATCH-CONVERT] Starting batch video conversion...", flush=True)
        ...
        field_mapping = {
            'id_vid_url': ('face_video', 'id_verification'),
            'indigency_vid_url': ('mayorIndigency_video', 'indigency'),
            'grades_vid_url': ('mayorGrades_video', 'grades'),
            'enrollment_certificate_vid_url': ('mayorCOE_video', 'coe'),
            'schoolId_vid_url': ('schoolId_video', 'school_id')
        }
        ...
        for record in records:
            ...
            for field_index, (db_field, (field_name, folder)) in enumerate(field_mapping.items()):
                video_url = video_urls[field_index]
                ...
                try:
                    # Skip if already MP4
                    if '.mp4' in video_url and 'converted' not in video_url:
                        print(f"[BATCH-CONVERT] Skipping {db_field} for {applicant_no} (already MP4)", flush=True)
                        continue
                    
                    print(f"[BATCH-CONVERT] Converting {db_field} for {applicant_no}: {video_url[:80]}...", flush=True)
                    
                    # Download from Supabase
                    response = supabase.storage.from_('document_videos').download(
                        video_url.split('/document_videos/')[1]
                    )
                    ...
                    # Update database
                    cursor.execute(f"UPDATE users SET {db_field} = %s WHERE applicant_no = %s", 
                                  (new_url, applicant_no))
                    db.commit()
        ...
        print(f"[BATCH-CONVERT] Completed: {converted_count} converted, {error_count} errors", flush=True)
