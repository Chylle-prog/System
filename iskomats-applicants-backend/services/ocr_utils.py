...
def verify_cor_fields(parsed_fields, raw_text, first_name, middle_name, last_name, **kwargs):
    ...
    if expected_course and str(expected_course).strip():
        ...
        course_ok = verify_course_strict(expected_course, raw_text, found_course)
        if not course_ok:
            failures.append(f"Course mismatch (Expected: '{expected_course}', Found in COR: '{found_course or 'Not found'}')")
