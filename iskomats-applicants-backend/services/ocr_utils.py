...
def verify_name_sequence(first_name, last_name, target_text, full_raw_text=None, middle_name=None):
    ...
    # Combine single-word first name with first word of multi-word middle_name (e.g. Alexie + Chyle Ortega -> Alexie Chyle)
    mid_parts = mid_clean.split()
    if len(first_clean.split()) == 1 and len(mid_parts) >= 2:
        first_clean = f"{first_clean} {mid_parts[0]}".strip()
